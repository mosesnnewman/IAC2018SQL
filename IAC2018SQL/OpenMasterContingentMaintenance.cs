...
{
    public partial class OpenMasterContingentMaintenance : Form
    {
        private Boolean lbEdit = false, lbAdd = false, lbILockedIt = false,lbFormClosing = false;
        ...
        public OpenMasterContingentMaintenance()
        {
            InitializeComponent();
        }
        ...
        private void setRelatedData()
        {
            ...
            if (MasteriacDataSet.MASTER.Rows.Count > 0)
            {
                ...
                Comment2textBox.Visible = false;
                Comment2textBox.Enabled = false;
                opnmcontTableAdapter.FillByAll(MasteriacDataSet.OPNMCONT);
                MacontPos = MACONTbindingSource.Find("MACONT_ACC_NO", MasterAccNoTextBox.Text);
                if (MasteriacDataSet.OPNMCONT.Rows.Count == 0 || MacontPos == -1)
                {
                    MACONTbindingSource.AddNew();
                    MACONTbindingSource.EndEdit();
                    MasteriacDataSet.OPNMCONT.Rows[MACONTbindingSource.Position].SetField<String>("MACONT_ACC_NO", MasterAccNoTextBox.Text);
                    MasterNamecomboBox.Text = MasteriacDataSet.MASTER.Rows[0].Field<String>("MASTER_NAME");
                    MasterAccNoTextBox.Text = MasteriacDataSet.MASTER.Rows[0].Field<String>("MASTER_ACC_NO");
                    MasteriacDataSet.OPNMCONT.Rows[MACONTbindingSource.Position].SetField<Nullable<DateTime>>("MACONT_POST_DATE", DateTime.Now.Date);

                    MACONTbindingSource.EndEdit();

                    Text = "Open Master Contingent Maintenance (ADD MODE)";
                    toolStripButtonDelete.Enabled = false;
                }
                else
                {
                    MACONTbindingSource.Position = MacontPos;
                    Text = "Open Master Contingent Maintenance (EDIT MODE)";
                    LockMcontRecord();
        ...
        private void LockMcontRecord()
        {
            if (lbFormClosing)
                return;
            Object loLockedBy = opnmcontTableAdapter.LockedBy(MasterAccNoTextBox.Text),
                   loLockedTime = opnmcontTableAdapter.LockTime(MasterAccNoTextBox.Text);
            ...
            {
                opnmcontTableAdapter.LockRecord(Program.gsUserID, MasterAccNoTextBox.Text);
        ...
        {
            MACONTbindingSource.EndEdit();
            if (MasteriacDataSet.OPNMCONT.Rows[MACONTbindingSource.Position].Field<Nullable<DateTime>>("MACONT_POST_DATE") == null)
            {
                MasteriacDataSet.OPNMCONT.Rows[MACONTbindingSource.Position].SetField<Nullable<DateTime>>("MACONT_POST_DATE", (Nullable<DateTime>)TextBoxPostDate.Value);
                MACONTbindingSource.EndEdit();
            }
            ...
            tableAdapConn.Open();
            opnmcontTableAdapter.Connection = tableAdapConn;
            tableAdapTran = opnmcontTableAdapter.BeginTransaction();
            opnmcontTableAdapter.Transaction = tableAdapTran;
            try
            {
                opnmcontTableAdapter.Update(MasteriacDataSet.OPNMCONT.Rows[0]);
            ...
            {
                opnmcontTableAdapter.UnlockRecord(MasterAccNoTextBox.Text);
                tableAdapConn.Close();
                tableAdapConn = null;
                tableAdapTran = null;
                toolStripButtonSave.Enabled = false;
                ...
                if (lbEdit)
                    lbEdit = false;
                if (lbAdd)
                    MasteriacDataSet.OPNMCONT.Clear();
                StartUpConfiguration();
                ActiveControl = MasterAccNoTextBox;
        ...
        {
            if (MasteriacDataSet.OPNMCONT.Rows.Count < 1)
                return;
            MACONTbindingSource.EndEdit();
            tableAdapConn = new System.Data.SqlClient.SqlConnection();
            tableAdapConn.ConnectionString = IAC2018SQL.Properties.Settings.Default.IAC2010SQLConnectionString;
            ...
            try
            {
                opnmcontTableAdapter.Delete(MasteriacDataSet.OPNMCONT.Rows[MACONTbindingSource.Position].Field<String>("MACONT_ACC_NO"));
                tableAdapTran.Commit();
            }
            catch (System.Data.SqlClient.SqlException ex)
            {
        ...
        private void OpenMasterContingentMaintenance_FormClosing(object sender, FormClosingEventArgs e)
        {
            lbFormClosing = true;
            if (MasteriacDataSet.OPNMCONT.Rows.Count < 1)
                return;
            ...
            loLockedBy = opnmcontTableAdapter.LockedBy(MasteriacDataSet.OPNMCONT.Rows[MACONTbindingSource.Position].Field<String>("MACONT_ACC_NO"));
            if (loLockedBy != null)
            {
                if ((String)loLockedBy == Program.gsUserID && lbILockedIt)
                    opnmcontTableAdapter.UnlockRecord(MasteriacDataSet.OPNMCONT.Rows[MACONTbindingSource.Position].Field<String>("MACONT_ACC_NO"));
            }
        }
